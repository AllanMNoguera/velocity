...
var querycapacidad = 'SELECT RUTA, HORA_PARTIDA, COUNT(R.BUS) AS TOTAL, TIQUETE, CAPACIDAD FROM (SELECT * FROM RUTA_TIQUETE GROUP BY TIQUETE) AS R JOIN BUS AS B ON R.BUS = B.BUS JOIN TIPO_BUS AS T ON B.TIPO_BUS = T.TIPO_BUS WHERE RUTA = ? AND HORA_PARTIDA = ? AND R.BUS = ? GROUP BY RUTA, HORA_PARTIDA, R.BUS;';
...
var pay = function(socket, checklist, total) {
	box.connect(function(conn) {
		...
		conn.query('SELECT TIQUETE FROM TIQUETE ORDER BY TIQUETE DESC LIMIT 1;',function(err, rows, fields) {
			...
			console.log('Payment rejected, bus full' + err);
			socket.emit('busfull');
		}
		}
		});
